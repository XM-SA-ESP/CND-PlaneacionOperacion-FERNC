...
class ServicioSolar:
    ...
    def ejecutar_calculos(self, df: pl.DataFrame, params: JsonModelSolar) -> Respuesta:
        ...
        (
            self.inversores_pvsystem,
            self.inversores_resultados,
        ...
        for i, k1 in enumerate(self.inversores_pvsystem.keys()):
            lista_array = []
            params_inversor = self.inversores_resultados[f"Inversor {i + 1} Array 1 POA"]["params_inv"].ParametrosInversor
            params_modulo = self.inversores_resultados[f"Inversor {i + 1} Array 1 POA"]["params_inv"].ParametrosModulo
            for k2, j in self.inversores_resultados.items():
                if f"Inversor {i + 1}" in k2:
                    lista_array.append(j["solucion_cec"])
            
            pvsys = self.inversores_pvsystem[k1]

            resultado = self.ejecutar_calculo_dc_ac(pvsys, lista_array, params_trans, params_inversor, params_modulo)

            inv_dict[f"Inversor {i + 1}"] = resultado

        energia_planta = self.ejecutar_calculo_energia(inv_dict)

        energia_planta = self.calculo_dc_ac.ajuste_potencia_con_ihf_perdidas(energia_planta, params_trans)
    ...
    def ejecutar_calculo_poa(self, df_dni_dhi: Pandas_Dataframe, params: JsonModelSolar):
        """
        ...
        for i, params_inv in enumerate(params.ParametrosConfiguracion.GrupoInversores):
            ...
            for j, params_array in enumerate(params_inv.EstructuraYConfiguracion.CantidadPanelConectados):
            ...
            inv_sys = pvlib.pvsystem.PVSystem(arrays=arrays, name=f"PVSys Inv {i + 1}")
    ...
    def generar_archivo_excel(
        self, energia_planta, energia_mes, energia_diaria, enficc: Resultado, eda: list[Resultado], nombre_archivo
    ):
        """
        ...
        self.cliente_azure.generar_archivo_excel_y_subir(
            diccionario_hojas, nombre_archivo, self.cliente_azure
        )
    ...
    def calculo_poa(
        ...
        Parámetros:
            ...
            montura (FixedMount | SingleAxisTrackerMount): Objeto pvlib con la configuración de montaje de los paneles solares.
