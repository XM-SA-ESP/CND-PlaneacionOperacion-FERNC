...
class TestCorreccionVelocidadParque(unittest.TestCase):
    ...
    def test_calculo_h_z_prima(self):
        ...
        self.assertAlmostEqual(z_prima, 20.0, places=2)
    ...
    def test_obtener_curvas(self):
        ...
        curva_vel, curva_coef, curva_vcth = self.correccion_velocidad._CorreccionVelocidadParque__obtener_curvas(
            modelo
        )
        self.assertTrue(isinstance(curva_vel, np.ndarray))
        self.assertTrue(isinstance(curva_coef, np.ndarray))
        self.assertTrue(isinstance(curva_vcth, np.ndarray))

    def test_correccion_velocidad_parque_eolico(self):
        
        aero1 = Aerogenerador(id_torre="Torre1", latitud=360.0, modelo="modelo_test", longitud=1.0,
                              elevacion=200.0, id_aero=1, df=pd.DataFrame({"VelocidadViento": [200, 200, 300], "DireccionViento": [100, 200, 300], "DenBuje": [100, 200, 300]}, index=['2012-01-01', '2014-04-01', '2013-07-01']))

        aero2 = Aerogenerador(id_torre="Torre2", latitud=100.0, modelo="modelo_test", longitud=3.0,
                              elevacion=100.0, id_aero=2, df=pd.DataFrame({"VelocidadViento": [200, 300, 500], "DireccionViento": [100, 300, 500], "DenBuje": [100, 200, 300]}, index=['2012-01-01', '2014-04-01', '2013-07-01']))

        cf = [
                CurvasDelFabricante(
                    SerieVelocidad=1.0, SeriePotencia=3.0, SerieCoeficiente=2.0, SerieVcthCorregida=3),
                CurvasDelFabricante(
                    SerieVelocidad=2.0, SeriePotencia=3.6, SerieCoeficiente=1.0, SerieVcthCorregida=3)
            ]
        ...
        for v, aero_id in self.correccion_velocidad.correccion_velocidad_parque_eolico(
            fecha = '2012-01-01',
            ordenamiento = ["aero_id_1", "aero_id_2"],
            aerogeneradores = {"aero_id_1": aero1, "aero_id_2": aero2},
        ...
        self.assertEqual(result_list, [(2, 79.86748121993065)])
