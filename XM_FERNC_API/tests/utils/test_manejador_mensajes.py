...
from utils.manejador_mensajes import (
    procesar_mensaje,
    enviar_resultados,
...
class TestManejadorMensajes(unittest.TestCase):
    def setUp(self):
        self.json_string = f'''
            {{
    ...
    @mock.patch("utils.manejador_mensajes.enviar_resultados")
    @mock.patch("utils.manejador_mensajes.switch_dict")
    ...
    @mock.patch("utils.manejador_mensajes.enviar_mensaje_sb_transversal")
    def test_enviar_resultados(
        self, mock_enviar_mensaje_sb, mock_enviar_resultados
    ):
