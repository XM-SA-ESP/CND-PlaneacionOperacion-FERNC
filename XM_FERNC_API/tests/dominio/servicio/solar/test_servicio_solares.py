...
class TestServicioSolar(TestCase):
    ...
    def test_generar_dataframe(self):
        """
        Genera un dataframe utilizando la instancia mock de ServicioSolar.

        Parámetros:
            self: La instancia de la clase de prueba que contiene el método de prueba.

        Retorna:
            resultado: El resultado de invocar el método generar_dataframe.

        Afirmaciones:
            AssertEqual: Compara el resultado con el dataframe esperado.
        """

        # Configuracion de instancia Mock ServicioSolar
        self.servicio_solar_mock.generar_dataframe.return_value = self.df

        # Invocacion del metodo generar_dataframe
        resultado = self.servicio_solar.generar_dataframe("test_blob_name")

        # AssertEqual entre el resultado y el df esperado
        self.assertEqual(resultado.frame_equal(self.df), True)
    ...
    @mock.patch("pvlib.irradiance.get_total_irradiance")
    @mock.patch("pvlib.bifacial.pvfactors.pvfactors_timeseries")
    ...
    @mock.patch("pvlib.ivtools.sdm.fit_cec_sam")
    @mock.patch("pvlib.pvsystem.calcparams_cec")
    ...
    def test_generar_archivo_excel(self):
        ...
        self.servicio_solar_instancia.cliente_azure.generar_archivo_excel_y_subir.assert_called_with(
            diccionario_hojas, nombre_archivo, mock_cliente_azure
        )
    ...
    @patch.object(ClienteAzure, "__init__", return_value=None)
    ...
    def test_generar_dataframe(self, mock_cliente_azure_init, mock_archivo_leer):
        ...
        # Asegura que ClienteAzure se haya instanciado con el nombre del blob correcto
        mock_cliente_azure_init.assert_called_once_with(blob=nombre_blob)
